...
xls_file = pd.ExcelFile('/Users/theophanegregoir/Desktop/SportsCoding/Tennis/2002.xls')
temp = dt.datetime(1899, 12, 30)    # Note, not 31st Dec but 30th!

### Feature and Label selection
features_selection = ['Date','Tournament','Round','Best of','Surface','Court','Loser','WRank','LRank']
...
for i in range(2003,2020):
    ...
    txt = str(i)
    if (i <= 2012):
        add_file = pd.ExcelFile('/Users/theophanegregoir/Desktop/SportsCoding/Tennis/' + str(i) + '.xls')
        add_part = add_file.parse(txt)[total_selection].dropna(axis=0)
    else:
        add_file = pd.ExcelFile('/Users/theophanegregoir/Desktop/SportsCoding/Tennis/' + str(i) + '.xlsx')
        add_part = add_file.parse(txt)[total_selection].dropna(axis=0)
...
s = pd.Series([k for k in range(48748)])
full.set_index(s)
full['Winner_AB'] = 0
...
new_features_selection = ['Date','Tournament','Round','Best of','Surface','Court','PlayerA','PlayerB','RankA','RankB']
...
### ADABOOST CLASSIFIER
t0 = time()
parameters = {'n_estimators':[10,30], 'learning_rate':[0.2,0.6]}
ada = AdaBoostClassifier()
clf = GridSearchCV(ada, parameters, cv=8)
c, r = labels_train.shape
clf.fit(features_train,labels_train.values.reshape(c,))
t1 = time()
...
#t2 = time()
#parameters = {'n_estimators':[10,25,50], 'max_depth':[10,25,50], 'min_samples_split':[3,5,10,15]}
#forest = RandomForestClassifier()
#clf2 = GridSearchCV(forest, parameters)
#c, r = labels_train.shape
#clf2.fit(features_train,labels_train.values.reshape(c,))
